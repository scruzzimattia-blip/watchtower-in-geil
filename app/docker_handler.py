...
class DockerHandler:
    ...
    async def get_watchable_containers(self):
        ...
        sorted_watchable = await self.sort_containers_by_dependencies(watchable)

        logger.info(f"Es wurden {len(sorted_watchable)} zu ueberwachende Container gefunden.")
        return sorted_watchable

    async def sort_containers_by_dependencies(self, containers):
        """
        Sortiert Container topologisch basierend auf 'depends_on' Labels.
        """
        # Graphen aufbauen.
        watchable_infos = [await c.show() for c in containers]
        watchable_names = {info["Name"].lstrip("/") for info in watchable_infos}

        adj = {name: [] for name in watchable_names}
        in_degree = {name: 0 for name in watchable_names}

        for info in watchable_infos:
        ...
        name_to_container = {
            info["Name"].lstrip("/"): c for info, c in zip(watchable_infos, containers)
        }
        return [
            name_to_container[name] for name in sorted_names if name in name_to_container
        ]
    ...
    async def check_and_update(self, container, summary=None):
        ...
        try:
            ...
            if new_image_id != old_image_id:
                ...
                if Config.DRY_RUN:
                    logger.info(
                        f"[DRY RUN] Neustart von {container_name} wird uebersprungen."
                    )
                    if summary:
    ...
    async def _run_hook(self, container, label_key):
        ...
        try:
            ...
            config = {"Cmd": hook_cmd.split(), "AttachStdout": True, "AttachStderr": True}
                ...
                await network.connect(
                    {
                        ...
                        "EndpointConfig": {"Aliases": net_config.get("Aliases", [])},
                    }
                )
            except Exception as e:
        ...
        try:
            async for event in self.client.events():
                if event["Type"] == "container" and event["Action"] == "start":
                    ...
                    logger.info(
                        f"Event: Container {container_id} gestartet. Pruefe auf Updates..."
                    )
        except Exception as e:
